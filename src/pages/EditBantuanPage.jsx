...
const EditBantuanPage = () => {
  ...
  const loadBantuanData = async () => {
    try {
      setLoadingData(true);
      const response = await bantuanSosialAPI.getById(id);
      
      if (response.data.status === 'success') {
        const bantuan = response.data.data;
        ...
        setFormData({
          ...
          nominal: bantuan.nominal || '',
          kuota: bantuan.kuota || '',
          tanggal_mulai: formatDate(bantuan.tanggal_mulai),
          tanggal_selesai: formatDate(bantuan.tanggal_selesai),
          status: bantuan.status || 'Aktif',
          syarat_bantuan: bantuan.syarat_bantuan || '',
          dokumen_diperlukan: bantuan.dokumen_diperlukan || ''
        });
      } else {
        setMessage('Gagal memuat data bantuan sosial');
      }
    } catch (err) {
      console.error('Error loading bantuan sosial:', err);
      setMessage('Bantuan sosial tidak ditemukan');
      setTimeout(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await bantuanSosialAPI.update(id, submitData);

      if (response.data.status === 'success') {
        setMessage('Program bantuan berhasil diperbarui!');
        setTimeout(() => {
          navigate('/daftar-bantuan');
        }, 2000);
      } else {
        setMessage('Gagal memperbarui program bantuan');
      }
    } catch (err) {
      console.error('Error updating bantuan sosial:', err);
      ...
      if (err.response?.status === 422) {
        ...
        setMessage(err.response?.data?.message || 'Gagal memperbarui program bantuan');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Edit Program Bantuan</h1>
              <p className="text-slate-600">Perbarui informasi program bantuan sosial</p>
            </div>
          </div>
        </div>
...
export default EditBantuanPage;